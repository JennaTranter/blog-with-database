...
mongoose.connect("mongodb+srv://" + process.env.DB_USER + ":" + process.env.DB_KEY + "@cluster0.djrwo.mongodb.net/" + process.env.DB_NAME, {
...
const defaultPosts = [homeStartingContent];
    ...
    if (foundPosts.length === 0) {
      Post.insertMany(defaultPosts, function(err) {
        if (err) {
          ...
          console.log("Successfully saved starting content to DB.");
        }
      });
      res.redirect("/");
    } else {
...
app.post("/compose", function(req, res) {
  ...
  if (postName && postBody) {
    ...
    console.log("Posts must have a title and body.");
